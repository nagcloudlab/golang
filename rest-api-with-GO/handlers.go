...
func DeleteHandler(rw http.ResponseWriter, r *http.Request) {
	...
	t := db.FindUserID(intID)
	if t.Username != "" {
		log.Println("About to delete:", t)
		deleted := db.DeleteUser(intID)
		if deleted {
...
func GetAllHandler(rw http.ResponseWriter, r *http.Request) {
	...
	err := SliceToJSON(db.ListAllUsers(), rw)
	if err != nil {
		log.Println(err)
		rw.WriteHeader(http.StatusBadRequest)
		return
...
func GetIDHandler(rw http.ResponseWriter, r *http.Request) {
	...
	t := db.FindUserUsername(username)
	if t.ID != 0 {
		err := t.ToJSON(rw)
		if err != nil {
			rw.WriteHeader(http.StatusBadRequest)
...
func UpdateHandler(rw http.ResponseWriter, r *http.Request) {
	...
	log.Println(users)
	t := db.FindUserUsername(users[1].Username)
...
func LoginHandler(rw http.ResponseWriter, r *http.Request) {
	...
	t := db.FindUserUsername(user.Username)
...
func LoggedUsersHandler(rw http.ResponseWriter, r *http.Request) {
	...
	err = SliceToJSON(db.ReturnLoggedUsers(), rw)
	if err != nil {
		log.Println(err)
		rw.WriteHeader(http.StatusBadRequest)
		return
